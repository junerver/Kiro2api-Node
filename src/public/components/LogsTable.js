window.LogsTable = function LogsTable(props) {
        ...
        <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
                {props.logs.map((log, index) => {
                    const streamTag = log.stream === null
                    ...
                    return (
                        <tr key={index} className={`hover:bg-gray-50 transition ${index % 2 === 1 ? 'bg-gray-50/50' : ''}`}>
                            <td className="px-4 py-3 text-sm text-gray-600">{new Date(log.timestamp).toLocaleString()}</td>
                            <td className="px-4 py-3 text-sm text-gray-900">{log.accountName}</td>
                            <td className="px-4 py-3">{streamTag}</td>
                            <td className="px-4 py-3 text-xs text-gray-500" dangerouslySetInnerHTML={{ __html: formatModelDisplay(log.model, log.upstreamModel) }}></td>
                            <td className="px-4 py-3 text-xs text-gray-600">{log.apiKeyName || <span className="text-gray-400">--</span>}</td>
                            <td className="px-4 py-3 text-sm text-gray-600">{log.inputTokens || 0}</td>
                            <td className="px-4 py-3 text-sm text-gray-600">{log.outputTokens || 0}</td>
                            <td className="px-4 py-3 text-sm text-gray-600">{log.durationMs}ms</td>
